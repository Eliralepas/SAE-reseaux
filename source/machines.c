...
void ip_to_str(adresse_IP IP, char *str){
    sprintf(str, "%02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX", IP.ip[0], IP.ip[1], IP.ip[2], IP.ip[3]);
}
...
void str_to_ip(adresse_IP * IP, char *str){
    sscanf(str, "%02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX", &IP->ip[0], &IP->ip[1], &IP->ip[2], &IP->ip[3]);
}